...
// Returns parsed Eurovision entries from the official site, with Wikipedia fallback.
...
export async function parseEurovision(url: string): Promise<ParseResult> {
  const r = await fetch(url, {
    headers: { 'user-agent': BROWSER_UA, accept: 'text/html' },
  });
  if (r.status === 200) {
    const html = await r.text();
    const entries = extractEurovision(html);
    if (entries.length > 0) {
      return { entries, httpStatus: 200, source: 'eurovision' };
    }
  }
  const wiki = await fetch('https://en.wikipedia.org/wiki/Eurovision_Song_Contest_2026', {
    headers: { 'user-agent': BROWSER_UA },
  });
  return {
    entries: extractWikipedia(await wiki.text()),
    httpStatus: r.status,
    source: 'wikipedia',
  };
}
...
function extractEurovision(html: string): ParsedEntry[] {
  ...
  for (const m of html.matchAll(re)) {
    entries.push({
      ...
      artist: m[3],
      song: m[4],
      runningOrder: m[5] ? parseInt(m[5]) : null,
    });
  }
  return entries;
}
